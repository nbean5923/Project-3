...
This is a significant amount of money saved for the company, but we should also consider the opportunity cost of the employees we incorrectly predicted. The only added opportunity cost from this method comes from the false positives, because these are employees that should be fired that we are instead keeping and waiting for them to fire. This cost is a hard number to determine, but the breakeven cost is simple:

```{r}
#Saved money per quitter divided by probability a positive prediction is false
saved_money_per_quitter/(5/16)
```

As long as the company loses less than $54,487 by keeping an undesirable employee in the hopes that they quit, this method is profitable. Below is the overall expected amount of saved money per employee that we are expecting to quit:

```{r}
#Saved money per quitter multiplied by probability a positive prediction is true
saved_money_per_quitter*(11/16)
```

Each time the company has an undesirable employee that they believe will quit, they will save $11,706 on average.