...
employees.knn <- train(as.factor(attrition_yes) ~ ., data = employees.train.matrix, method = "knn",
           ...
           tuneGrid = grid_knn)
employees.knn
write.csv(employees.knn$results, file = "employees.knn.csv")
```
...
write.csv(logreg.preds, file = "logreg.preds.csv")
```

# ANN
```{r}
employees.ann <- neuralnet(attrition_yes ~ ., data = employees.train.matrix, hidden = 1, stepmax = 1e+8)
...
agreement <- employees.prediction == employees.test$Attrition
table(agreement)
prop.table(table(agreement))

employees.classifier.rbf <- ksvm(Attrition ~ .,data = employees.train,kernel = "rbfdot")
employees.prediction.rbf <- predict(employees.classifier.rbf,employees.test)

agreement.rbf <- employees.prediction.rbf == employees.test$Attrition
table(agreement.rbf)
prop.table(table(agreement.rbf))
write.csv(employees.prediction.rbf, file = "svm.preds.csv")
```
...
employees.tree <- train(Attrition ~ ., data = employees.train, method = "C5.0",
           ...
           tuneGrid = grid_tree)
employees.tree
write.csv(employees.tree$results, file = "decision.tree.csv")
```
